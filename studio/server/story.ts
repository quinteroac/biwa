...
import type { StudioStoryFile, StudioStoryFolder, StudioStoryListResponse, StudioStoryPreviewLine, StudioStoryResponse } from '../shared/types.ts'
...
function resolveStoryFolder(gameId: string, path: string): string {
  const baseDir = storyDir(gameId)
  const normalizedPath = normalizeStoryPath(path)
  if (!normalizedPath || normalizedPath.endsWith('.ink')) throw new Error('Story folder path must be a directory.')
  const resolved = normalize(join(baseDir, normalizedPath))
  if (!resolved.startsWith(baseDir)) throw new Error('Story folder path escapes the project story directory.')
  return resolved
}
...
function walkStoryFolders(dir: string, baseDir = dir): StudioStoryFolder[] {
  if (!existsSync(dir)) return []
  const folders: StudioStoryFolder[] = []
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue
    const fullPath = join(dir, entry.name)
    const path = relative(baseDir, fullPath).replace(/\\/g, '/')
    const locale = path.split('/')[0] ?? ''
    folders.push({ path, locale })
    folders.push(...walkStoryFolders(fullPath, baseDir))
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^[*+]/.test(trimmed)) {
      preview.push({
        line: i + 1,
        kind: 'choice',
        text: trimmed.replace(/^[*+]+\s*/, '').replace(/^\[(.*)\]$/, '$1').replace(/\s*->.*$/, '').trim(),
      })
      continue
    }
...
export async function listStoryEntries(gameId: string): Promise<StudioStoryListResponse> {
  await validateGame(gameId)
  const baseDir = storyDir(gameId)
  return storyEntries(baseDir)
}

function storyEntries(baseDir: string): StudioStoryListResponse {
  return {
    files: walkInkFiles(baseDir),
    folders: walkStoryFolders(baseDir),
  }
}

export async function createStoryFolder(gameId: string, path: string): Promise<StudioStoryListResponse> {
  await validateGame(gameId)
  mkdirSync(resolveStoryFolder(gameId, path), { recursive: true })
  return storyEntries(storyDir(gameId))
}

export async function renameStoryFolder(gameId: string, fromPath: string, toPath: string): Promise<StudioStoryListResponse> {
  await validateGame(gameId)
  const sourcePath = resolveStoryFolder(gameId, fromPath)
  const targetPath = resolveStoryFolder(gameId, toPath)
  if (!existsSync(sourcePath)) throw new Error(`Story folder not found: ${fromPath}`)
  if (existsSync(targetPath)) throw new Error(`Story folder already exists: ${toPath}`)
  mkdirSync(dirname(targetPath), { recursive: true })
  renameSync(sourcePath, targetPath)
  return storyEntries(storyDir(gameId))
}

export async function deleteStoryFolder(gameId: string, path: string): Promise<StudioStoryListResponse> {
  await validateGame(gameId)
  const folderPath = resolveStoryFolder(gameId, path)
  if (!existsSync(folderPath)) throw new Error(`Story folder not found: ${path}`)
  rmSync(folderPath, { recursive: true, force: true })
  return storyEntries(storyDir(gameId))
}

export async function renameStoryFile(gameId: string, fromPath: string, toPath: string): Promise<StudioStoryResponse> {
  await validateGame(gameId)
  const sourcePath = resolveStoryFile(gameId, fromPath)
  const targetPath = resolveStoryFile(gameId, toPath)
  if (!existsSync(sourcePath)) throw new Error(`Story file not found: ${fromPath}`)
  if (existsSync(targetPath)) throw new Error(`Story file already exists: ${toPath}`)
  mkdirSync(dirname(targetPath), { recursive: true })
  renameSync(sourcePath, targetPath)
  return readStoryFile(gameId, toPath)
}

export async function deleteStoryFile(gameId: string, path: string): Promise<StudioStoryListResponse> {
  await validateGame(gameId)
