...
/**
 * Controls background music playback by listening to `engine:bgm` events
...
export class BgmController {
  ...
  constructor(bus: EventBus<any>) {
    this.#bus = bus
    this.#unsubscribe = this.#bus.on<TagCommand>('engine:bgm', cmd => this.#handle(cmd))
  }
  ...
  /**
   * Starts a BGM track. Stops any currently playing BGM first.
  ...
  #play(cmd: TagCommand): void {
    ...
    this.#audio = audio
    void audio.play().catch(e => console.warn('[BgmController] play() failed:', e))
  }

  /**
   * Stops the currently playing BGM track immediately.
   */
  #stop(): void {
    if (this.#audio) {
      this.#audio.pause()
  ...
  /**
   * Adjusts the BGM volume without interrupting playback.
